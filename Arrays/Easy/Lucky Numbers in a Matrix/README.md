...
Lucky Numbers in a Matrix — Revision Points
Lucky number: Minimum in its row + Maximum in its column.
Traverse every element matrix[i][j].
To check row minimum → matrix[i][k].
To check column maximum → matrix[k][j].
If both conditions are true → add the element to the answer.
Row: i fixed, k changes.
Column: j fixed, k changes.
Your approach: O(m × n × (m + n)) time.
Extra space: O(1) excluding the result list.
